...
function check_args()
{
    ...
    then
        ...
        echo -e "[!] Available tasks: [css, scss, js, copy, all, watch]\n"
        exit -1
    elif [[ $2 == '' ]]
    then
        echo -e "[!] Usage: $0 -t <task>"
...
function main()
{
    ...
    do
        ...
        if [[ $task == 'js' || $task == 'css' || $task == 'scss' || $task == 'copy' || $task == 'all' || $task == 'watch' ]]
        then
            ...
            if [[ $task == 'js' ]]; then
                ...
                run_scss
                if [[ $? == 0 ]]; then
                    echo -e "[+] Tasks css, js, scss and copy are done successfully\n"
                fi
